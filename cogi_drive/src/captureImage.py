...
class CaptureImage:
    def __init__(self, rospy):
        ...
        self.imageSeq = 0
        self.calibratedImage = np.empty(shape=[0])
        self.calibratedImageSeq = 0
        self.birdEyeViewImage  = np.empty(shape=[0])
        self.birdEyeViewImageSeq = 0
        self.laneHighlightedBirdEyeViewImage = np.empty(shape=[0])
        self.laneHighlightedBirdEyeViewImageSeq = 0
        ...
        self.calibrationMatrix = np.array([
            ...
            [   0.        ,    0.        ,    1.        ]])
        self.calibrationDist = np.array(
            [[-0.34825133,  0.13658961,  0.0004364 , -0.00051536, -0.02596215]])
        ...
        self.birdEyeViewCalibrationDist = np.array(
            [[  1.17224820e+01,  -7.30422723e+02,  -4.50227100e-01,
                4.88336487e-01,  -5.44254659e+00]])
        self.calibrationMatrix=np.array( [
            [ 359.55954828,    0.         , 304.93780631],
            [   0.   ,       358.45714241,  224.95234163],
            [   0.   ,         0.          ,  1.        ]])

        self.calibrationDist=np.array( [
            [-0.30956134 , 0.09548415 , 0.00166552 , 0.00114415 -0.01386452]])

        self.birdEyeViewMatrix=np.array( [
            [ -2.48496108e-01,  -1.32531290e+00 ,  3.99251713e+02],
        ...
        self.birdEyeViewCalibrationDist=np.array( [
            [  1.17224820e+01,  -7.30422723e+02,  -4.50227100e-01 ,  4.88336487e-01
            , -5.44254659e+00]])
    ...
    def getCalibratedImage(self):
        ...
        self.calibratedImage = cv2.undistort(
                rawImage, self.calibrationMatrix, 
                self.calibrationDist, None, self.calibrationMatrix)
        return self.calibratedImage
...
if( __name__ == "__main__" ):
    ...
    #capture_image.calibrationMatrix, capture_image.calibrationDist = capture_image.getCalibrationMatrix(capture_image.getImage)
    print "calibrationMatrix=",
    print capture_image.calibrationMatrix
    print "calibrationDist=",
    print capture_image.calibrationDist
    
    #capture_image.getBirdEyeViewMatrix()
    print "birdEyeViewMatrix=",
    print capture_image.birdEyeViewMatrix
    
    #while(True):
    #    cv2.imshow('calibrated image', capture_image.getCalibratedImage())
    #    if cv2.waitKey(1) & 0xFF == ord('q'):
    #        break

    #capture_image.getBirdEyeViewCalibrationMatrix()
    #while(True):
    #    cv2.imshow('bird eye view image', capture_image.getBirdEyeViewImage())
    #    if cv2.waitKey(1) & 0xFF == ord('q'):
    #        break
    
    print "birdEyeViewCalibrationMatrix=",
    print capture_image.birdEyeViewCalibrationMatrix
    print "birdEyeViewCalibrationDist=",
    print capture_image.birdEyeViewCalibrationDist

    while(True):
        #cv2.imshow('bird eye view image', capture_image.getCalibratedBirdEyeViewImage())
        cv2.imshow('bird eye view image', capture_image.getBirdEyeViewImage())
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
